...
Route::group(['prefix' => 'v2/auth', 'middleware' => ['app_language']], function() {
    ...
    //Custom Route
    //Login with otp
    Route::post('login-with-otp', 'App\Http\Controllers\Api\V2\LoginWithOtpController@sendOtp');
...
Route::group(['prefix' => 'v2', 'middleware' => ['app_language']], function() {
    ...
    Route::post('reviews/submit', 'App\Http\Controllers\Api\V2\ReviewController@submit')->name('api.reviews.submit');
    ...
    Route::get('home-products', 'App\Http\Controllers\Api\V2\ProductController@homeProducts');
    Route::get('sidebar-data', 'App\Http\Controllers\Api\V2\ProductController@sidebarData');
    Route::get('filter-products', 'App\Http\Controllers\Api\V2\ProductController@filterProduct');
    Route::post('order-store', 'App\Http\Controllers\Api\V2\OrderController@nuxtStore');
    Route::get('new-arrivals', 'App\Http\Controllers\Api\V2\ProductController@newArrivals');// No method
    ...
    Route::get('youtube-video/{slug}', 'App\Http\Controllers\Api\V2\KireiYoutubeController@show');

    // Route::apiResource('doctors', 'App\Http\Controllers\Api\V2\DoctorController')->only('index', 'show');
    Route::apiResource('beauty-blog-categories', 'App\Http\Controllers\Api\V2\BeautyBlogCategoryController')->only('index');
    Route::get('beauty-blogs', 'App\Http\Controllers\Api\V2\BeautyBlogController@index');
    Route::get('beauty-blog-features/', 'App\Http\Controllers\Api\V2\BeautyBlogController@feature');
    Route::get('beauty-blog/{id}', 'App\Http\Controllers\Api\V2\BeautyBlogController@show');
    Route::apiResource('community-hashtags', 'App\Http\Controllers\Api\V2\HashtagController')->only('index','show');
    Route::apiResource('subscribers', 'App\Http\Controllers\Api\V2\SubscriberController')->only('index','store','update'); //no controller
    Route::get('contact-us', 'App\Http\Controllers\Api\V2\ContactUsController@index');//no controller
    Route::post('contact-us', 'App\Http\Controllers\Api\V2\ContactUsController@store');//no controller


    // Route::get('community-posts', 'App\Http\Controllers\Api\V2\CommunityPostController@index');
    Route::get('community-comments/{post_id}', 'App\Http\Controllers\Api\V2\CommunityCommentController@show');
    Route::get('community-like/{id}', 'App\Http\Controllers\Api\V2\CommunityLikeController@show');
    Route::get('community-posts/{id}', 'App\Http\Controllers\Api\V2\CommunityPostController@show');

    Route::get('community-posts', 'App\Http\Controllers\Api\V2\CommunityPostController@index');
    Route::post('community-post-create', 'App\Http\Controllers\Api\V2\CommunityPostController@store')->middleware('auth:sanctum');
    ...
    //Fetch City
    Route::post('/fetch-cities', 'App\Http\Controllers\Api\V2\StateController@fetchCities');
