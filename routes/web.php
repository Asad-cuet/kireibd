...
Route::get('/test-update', function () {
    ...
    // foreach ($products as $index => $item) {
        ...
        // Step 01
        // echo "<br>Initial: " . $item->photos;
        // $photos = explode(',', $item->photos);
        // $new_photos = '';
        // foreach ($photos as $key => $photo) {
        //     if (trim($photo) == trim($item->thumbnail_img)) {
        //         echo "<br>Matched: " . $photo;
        //     } else {

        //         echo "<br>Not Matched: " . $photo;

        //         $update_upload = DB::table('uploads')->where('id',$photo)->update(['file_original_name'=>$item->name]);
        //         $upload = DB::table('uploads')->find($photo);
        //         if ($upload != null) {
        //             $new_upload = DB::table('uploads')->where('file_name', $upload->file_name)->first();
        //             echo "<br>Item found: " . $new_upload->id;
        //             if ($new_photos != '') {
        //                 $new_photos .= ',' . $new_upload->id;
        //             } else {
        //                 $new_photos = $new_upload->id;
        //             }
        //         } else {
        //             echo "<br>Not Found: " . $photo;
        //         }
        //     }
        // }

        // echo "<br>Final: " . $new_photos;
        // $update = DB::table('products')->where('id', $item->id)->update(['photos' => $new_photos]);

        // Step 02

        // echo "<br>Initial: " . $item->thumbnail_img;

        // $upload = DB::table('uploads')->find($item->thumbnail_img);
        // if ($upload != null) {
        //     $new_upload = DB::table('uploads')->where('file_name', $upload->file_name)->first();
        //     echo "<br>Found: " . $new_upload->id;
        //     $update = DB::table('products')->where('id', $item->id)->update(['thumbnail_img' => $new_upload->id]);
        // } else {

        //     echo "<br>Not Found: " . $item->thumbnail_img;
        //     echo "<br>Photos: " . $item->photos;
        //     $update = DB::table('products')->where('id', $item->id)->update(['thumbnail_img' => '', 'published' => 0, 'approved' => 0]);
        // }
    // }
    // dd('Success');
});
