...
function directOrderAddCart($data, $user_id)
    {
...
if (!function_exists('getPurchasePrice')) {
    function getPurchasePrice($unit_price,$discount,$discount_type)
    {
...
if (!function_exists('formatCategory')) {
    function formatCategory($category,$product_id)
    {
        ...
        if($category!=''){
            ...
            $item->parent_name=$category->parent_id;
            $pivot->product_id=$product_id;
            $pivot->product_category_id=$category->id;
...
if (!function_exists('formatBrand')) {
    function formatBrand($brand,$product_id)
    {
        ...
        if($brand!=null){
            ...
            $pivot->product_id=$product_id;
            $pivot->product_brand_id=$brand->id;
...
if (!function_exists('formatTags')) {
    function formatTags($tags,$product_id)
    {
        $keys=explode(",",$tags);
        $all_tags=[];
...
if (!function_exists('formatBlogCategories')) {
    function formatBlogCategories($category, $blog_id)
    {        
        $categories=[];
        $item=new stdClass();
        $pivot=new stdClass();
        $item->name=$category->name;
        $item->slug=$category->slug;
        $pivot->blog_id=$blog_id;
        $pivot->blog_category_id=$category->id;
...
if (!function_exists('formatBlogPictures')) {
    function formatBlogPictures($photo_paths,$product_id)
    {

        $photos = [];
        if (!empty($photo_paths)) {

            foreach (explode(',', $photo_paths) as $file_id){
                
                $item=new stdClass();
                $pivot=new stdClass();

                if (($asset = \App\Models\Upload::find($file_id)) != null) {
                    $item->url=$asset->external_link == null ? my_asset($asset->file_name) : $asset->external_link;
                }else{
                    $item->url=null;
                }
                $item->width=1280;
                $item->height=500;
                $pivot->related_id=$product_id;
                $pivot->upload_file_id=$file_id;
...
if (!function_exists('formatLargePictures')) {
    function formatLargePictures($photo_paths, $thumbnail_id, $product_id)
    {
        ...
        if (!empty($thumbnail_id)) {
                ...
                if (($asset = \App\Models\Upload::find($thumbnail_id)) != null) {
                    $item->url=$asset->external_link == null ? my_asset($asset->file_name) : $asset->external_link;
                }else{
                    $item->url=null;
                }
                ...
                $item->height=800;
                $pivot->related_id=$product_id;
                $pivot->upload_file_id=$thumbnail_id;
...
if (!function_exists('formatSmallPictures')) {
    function formatSmallPictures($photo_paths, $thumbnail_id, $product_id)
    {
