...
class ReviewController extends Controller
{
    ...
    {
        return new ReviewCollection(Review::with('reviewReplay')->orderBy('id', 'desc')->where('product_id', $id)->where('status', 1)->orderBy('updated_at', 'desc')->paginate(10));
    }
    ...
    public function submit(Request $request)
    {
        ...
        */
        ...
        $review->comment_email = auth()->user()->email ?? $request->email;
        $review->comment_author = auth()->user()->name ?? $request->name;
