...
class ProductService
{
    public function store(array $data)
    {
        ...
        if ($collection['key_ingredients'][0] != null) {
            foreach (json_decode($collection['key_ingredients'][0]) as $key => $key_ingredient) {
                array_push($key_ingredients, $key_ingredient->value);
            }
        }
        $collection['key_ingredients'] = implode(',', $key_ingredients);

        $skin_types = array();
        if ($collection['skin_types'][0] != null) {
            foreach (json_decode($collection['skin_types'][0]) as $key => $skin_type) {
                array_push($skin_types, $skin_type->value);
            }
        }
        $collection['skin_types'] = implode(',', $skin_types);

        $good_for = array();
        if ($collection['good_for'][0] != null) {
            foreach (json_decode($collection['good_for'][0]) as $key => $item) {
                array_push($good_for, $item->value);
            }
        }
        $collection['good_for'] = implode(',', $good_for);
        ...
        $published = 1;
        if ($collection['button'] == 'unpublish' || $collection['button'] == 'preview') {
            $published = 0;
        }
    ...
    public function update(array $data, Product $product)
    {
        ...
        $good_for = array();
        if ($collection['good_for'][0] != null) {
            foreach (json_decode($collection['good_for'][0]) as $key => $tag) {
                array_push($good_for, $tag->value);
            }
        }
        $collection['good_for'] = implode(',', $good_for);
